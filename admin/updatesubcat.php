...
$q=$_GET['q'];
$m=$_GET['m'];
$query3 = "select * from subcategory  where subCategory='$q'";

$result3 = mysqli_query($CONNECTION, $query3);
$table_record3 = mysqli_fetch_array($result3);
$categoryName = $table_record3['subCategory'];
...
if(isset($_POST['submit1'])) {
    ...
    if($name!="") {
            ...
            if (in_array($ext, $valid_formats)) {
                if ($size < (345 * 777)) {
                    ...
                    if (move_uploaded_file($tmp, $path1 . $actual_image_name)) {
                        ...
                        if (empty($errors)) {
                            include_once('dbConnect.php');
                            $sql = "update  `subcategory` SET subCategory='$article_name',pic='$pic1',description='$description',status='$afterStatus' where subCategory='$q'";
                            $result = mysqli_query($CONNECTION, $sql);
                            echo'<style type="text/css">
                #idalert {
                    visibility: visible;
                }
            </style>';         echo "<script> window.open ('addsubcat.php?q=$m','_self',false);</script>";
        ...
        if (empty($errors)) {
            include_once('dbConnect.php');
            $sql = "update  `subcategory` SET subCategory='$article_name',description='$description',status='$afterStatus' where subCategory='$q'";
            $result = mysqli_query($CONNECTION, $sql);
            echo'<style type="text/css">
                #idalert {
                    visibility: visible;
...
<head>
    ...
    <meta charset="UTF-8">
    <title>Update Sub Category Name</title>
                ...
                <div class="col-xs-12">
                    ...
                    <div class="box box-info" id="form" >
                        <div class="box-header with-border">
                            <h3 class="box-title">Update Sub Category Form</h3>
